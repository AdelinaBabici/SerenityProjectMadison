package com.utils;

import org.yecht.Data;

public class Constants {
    public static final String URL = "http://qa2.dev.evozon.com/";
    public static final String EMAIL = "ab@yahoo.com";
    public static final String PASS = "aaaaaa";
    public static final String USER_NAME= "aadsa asda";

    public static  final String USER_TO_UPDATE_PASSWORD = "eminem";
    public static  final String USER_TO_UPDATE_USERNAME = "mona@yahoo.com";

    public  static  final int TIMEOUT = 15000;

    public static final String URL_MAGENTO = "http://qa2.dev.evozon.com/index.php/admin";
    public static final String USER_NAME_MAGENTO ="admin";
    public static final String PASSWORD_MAGENTO ="parola11";


}
